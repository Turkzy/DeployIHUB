...
export const createTeam = async (req, res) => {
    ...
    const pdfFile = req.files.pdf; // New PDF file

    try {
        // Upload image to Cloudinary
        const imageResult = await cloudinary.v2.uploader.upload(imageFile.tempFilePath || imageFile.path, {
            folder: 'team_images'
        });
        ...
        let pdfUrl = null;
        if (pdfFile) {
            const pdfResult = await cloudinary.v2.uploader.upload(pdfFile.tempFilePath || pdfFile.path, {
                folder: 'team_pdfs',
...
export const updateTeam = async (req, res) => {
    try {
        ...
        if (req.files && req.files.Imgurl) {
            ...
            if (team.Imgurl) {
                const imagePublicId = team.Imgurl.split('/').slice(-2)[0] + '/' + 
        ...
        if (req.files && req.files.pdf) {
            ...
            if (team.pdfUrl) {
                const pdfPublicId = team.pdfUrl.split('/').slice(-2)[0] + '/' + 
...
export const deleteTeam = async (req, res) => {
    try {
        ...
        res.status(200).json({ msg: "Team Member and associated files deleted successfully" });
    } catch (error) {
...
export const downloadPdf = async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${team.name}-document.pdf"`);
